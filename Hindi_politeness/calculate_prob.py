...
def init_model(model_name):
    if model_name == "hindibert":
        pretrained_name = 'monsoon-nlp/hindi-bert'
    ...
    elif model_name == "hindibert-large":
        pretrained_name = 'google/muril-large-cased'
        tokenizer = AutoTokenizer.from_pretrained(pretrained_name)
        model = AutoModelForCausalLM.from_pretrained(pretrained_name, output_hidden_states=True, output_attentions=True)
        model_lm = AutoModelWithLMHead.from_pretrained(pretrained_name).eval()
    elif model_name == "hindimgpt":
        pretrained_name = 'ai-forever/mGPT'
        tokenizer = AutoTokenizer.from_pretrained(pretrained_name)
        model = AutoModelForCausalLM.from_pretrained(pretrained_name, output_hidden_states=True, output_attentions=True)
        model_lm = AutoModelWithLMHead.from_pretrained(pretrained_name).eval()
...
if __name__ == '__main__':
    ...
    '''
    inputfiles = ['hindi_polite1.txt', 'hindi_polite2.txt', 'hindi_polite3.txt', 'hindi_polite4.txt', 'hindi_polite_base_nosubj.txt', 'hindi_polite_base_withsubj.txt']
    print(sys.argv[1])
    inputfile = inputfiles[int(sys.argv[1])]

    model_names = ["hindibert", "hindibert-large", "hindimgpt"]
    for model_name in model_names:
        ...
        for input in open(inputfile):
            ...
            if len(sys.argv[1].split('_')) > 2:
                outfile.write(input_sent + ' prob_sent: ' + str(sent_pp) + ' prob_target: ' + str(mid1[-4]) + '\n')
            else:
                outfile.write(input_sent + ' prob_sent: ' + str(sent_pp) + ' prob_target: ' + str(mid1[-6]) + '\n')
